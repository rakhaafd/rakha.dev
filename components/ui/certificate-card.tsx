    ...
    <Card className={cn("p-3.5 sm:p-5 transition-colors duration-150 space-y-2 sm:space-y-3", className)}>
      <div className="flex items-start justify-between gap-2.5">
        <div className="space-y-1 pr-1">
            ...
            {certificate.category && certificate.category.length > 0 && (
              <div className="flex flex-wrap gap-1">
                {certificate.category.map((cat) => (
                  <Badge key={cat} variant="subtle" className="text-[10px] sm:text-[11px] px-1.5 py-0">
                    {cat}
                  </Badge>
                ))}
              </div>
          ...
          <p className="text-[11px] sm:text-xs text-muted-foreground font-serif">
            {certificate.issuer}
      ...
      {certificate.details && (
        <p className="hidden sm:block text-xs text-muted-foreground/80 leading-relaxed font-sans border-t border-border/30 pt-2.5">
          {certificate.details}
        </p>
      )}
    </Card>
